...
class _StudentGradesViewState extends State<StudentGradesView> {
    ...
    try {
      ...
      final gradesFutures = enrollmentsSnap.docs.map((enrollDoc) async {
        ...
        if (courseData.containsKey('gradingScheme')) {
          final scheme =
              courseData['gradingScheme'] as Map<String, dynamic>;
          gradingScheme =
      ...
      final allGrades =
          results.whereType<_CourseGrade>().toList();
      ...
      if (allGrades.isNotEmpty) {
        initialSemester = allGrades.map((g) => g.semester).reduce(
            (a, b) => a > b ? a : b);
      }
  ...
  int get _latestSemester {
    if (_allGrades.isEmpty) return 1;
    return _allGrades.map((g) => g.semester).reduce((a, b) => a > b ? a : b);
  }

  /// IPS semester terbaru
  double get _ipsLatest {
    final latestGrades =
        _allGrades.where((g) => g.semester == _latestSemester).toList();
    return _calcIPS(latestGrades);
  }

  /// Total SKS seluruh semester
  int get _totalSks => _allGrades.fold(0, (sum, g) => sum + g.sks);
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (ctx, i) {
          ...
          final semGrades =
              _allGrades.where((g) => g.semester == sem).toList();
  ...
  Widget _buildSemesterSummaryCard() {
    ...
    final ips = _calcIPS(semGrades);
    final semTotalSks = semGrades.fold(0, (sum, g) => sum + g.sks);

    Color ipsColor = Colors.red;
    if (ips >= 3.5) ipsColor = Colors.green;
    else if (ips >= 3.0) ipsColor = Colors.blue;
    else if (ips >= 2.5) ipsColor = Colors.orange;
    else if (ips >= 2.0) ipsColor = Colors.deepOrange;
            ...
            child: Column(
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: grade.gradeColor.withValues(alpha: 0.08),
  ...
  Widget _buildGradeBadge(_CourseGrade grade) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(10),
        border:
            Border.all(color: grade.gradeColor.withValues(alpha: 0.4)),
      ),
      child: Column(
